...
add_pdo_extension() {
  ...
  if check_extension "$pdo_ext"; then
    ...
    sudo rm -rf "$scan_dir"/*pdo.ini  && echo "extension=pdo.so" >> "$ini_file"
    if [ "$ext" = "mysql" ]; then
...
add_devtools() {
  if ! [ -e "/usr/bin/phpize$version" ] || ! [ -e "/usr/bin/php-config$version" ]; then
    update_ppa && $apt_install php"$version"-dev php"$version"-xml 
  fi
...
if [ "$existing_version" != "$version" ]; then
  if [ ! -e "/usr/bin/php$version" ]; then
    if [ "$version" = "8.0" ]; then
      ...
      update_ppa
      $apt_install php"$version" php"$version"-curl php"$version"-mbstring php"$version"-xml php"$version"-intl 
    fi
    status="Installed"
  else
    if [ "$update" = "true" ]; then
